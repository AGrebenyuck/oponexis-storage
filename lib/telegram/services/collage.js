...
export async function createStockCollage(batches, offset = 0) {
	// перед любой работой с canvas:
	const { createCanvas, loadImage, registerFont } = await getCanvasLib()

	// регистрируем шрифт (если ещё не зарегистрирован)
	try {
		registerFont('/var/task/fonts/Inter-Bold.ttf', {
			family: 'OponexisCollageFont',
		})
	} catch (e) {
		// если уже зарегистрирован — можно просто проглотить
		console.warn(
			'[collage] registerFont error (может быть уже зарегистрирован):',
			e?.message
		)
	}
